...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4">
    <div>
        <h1 class="h2 mb-1">ویرایش پرسنل</h1>
        <p class="text-muted mb-0">ویرایش اطلاعات {{ $staff->full_name }}</p>
    </div>
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">فرم ویرایش پرسنل</h5>
    </div>
    <div class="card-body">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('national_id') is-invalid @enderror" 
                               id="national_id" name="national_id" value="{{ old('national_id', $staff->national_id) }}" 
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                            ...
                            <option value="doctor" {{ old('role', $staff->role) == 'doctor' ? 'selected' : '' }}>دکتر</option>
                            <option value="secretary" {{ old('role', $staff->role) == 'secretary' ? 'selected' : '' }}>منشی</option>
                            <option value="assistant" {{ old('role', $staff->role) == 'assistant' ? 'selected' : '' }}>کمک‌کار</option>
                            <option value="nurse" {{ old('role', $staff->role) == 'nurse' ? 'selected' : '' }}>پرستار</option>
                            <option value="cleaner" {{ old('role', $staff->role) == 'cleaner' ? 'selected' : '' }}>نگهبان/تمیزکار</option>
                            <option value="other" {{ old('role', $staff->role) == 'other' ? 'selected' : '' }}>سایر</option>
                        </select>
                        @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('hourly_rate') is-invalid @enderror" 
                               id="hourly_rate" name="hourly_rate" value="{{ old('hourly_rate', $staff->hourly_rate) }}" 
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('hire_date') is-invalid @enderror" 
                               ...
                               value="{{ old('hire_date', $staff->hire_date ? verta($staff->hire_date)->format('Y/m/d') : '') }}" 
                               placeholder="مثلاً ۱۴۰۳/۰۹/۲۲" autocomplete="off" readonly>
                        <input type="hidden" id="hire_date_gregorian" name="hire_date" 
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('birth_date') is-invalid @enderror" 
                               ...
                               value="{{ old('birth_date', $staff->birth_date ? verta($staff->birth_date)->format('Y/m/d') : '') }}" 
                               placeholder="مثلاً ۱۴۰۳/۰۹/۲۲" autocomplete="off" readonly>
                        <input type="hidden" id="birth_date_gregorian" name="birth_date" 
